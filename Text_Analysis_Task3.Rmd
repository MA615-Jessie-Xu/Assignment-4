...
- verify my assessment of the text

Show the first 3 rows and 2-3 colunms of the data frame.
```{r}
library(knitr)
library(gutenbergr)
library(tidyverse)
...
I select out the most frequent words filter by sentiment "joy" from "nrc" lexicon, almost the same, but I can't figure out why the frequency of a word changed.
...
Almost the same, only the frequency of some words change a little.
...
I pick Heathcliff and Catherine as my character, and plot the number of times each one appears in each chapter and the number of times both characters appear in the same paragraphs.
...
- Split Sentences Using the Way I Learned by Myself
...
1. This EDA shows the distribution of the occurrence of Heathcliff and Catherine in each Chapter. Difference color refer different paragraphs.
...
ggplot(dt_CH, aes(ch)) +
  ...
  labs(title = "Distribution of the Appearance of Heathcliff and Catherine in each Chapter")+
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(fill = "Paragraph Number") +
  xlab("Chapter Number")+theme_bw()
```
2. Word Cloud:Plot the most frequent words which has real meaning in the whole passage, instead of stop and custum words or personal pronouns.
...
As you see, I try to make a tree diagram to plot the logical connection between chapters. But I can't use the function like  <tnum.makeTnumPhraseGraph> or<tnum.plotGraph> offered by the "tnum---instructions-and-examples-v1.0.pdf" and other tutorial about tnum. Even I already install the "tnum" package, the error said I can't use the function. I would like to figure it out later.